...
def token(client, user, db):
    ...
    response = client.post(
        "/api/token/", {"username": user.username, "password": "testpass"}
    )
    assert (
        response.status_code == 200
    ), response.content  # Ensure token request succeeds
...
def test_create_anemometer(client, token):
    ...
    response = client.post(
        "/api/anemometers/",
        {"name": "Test", "latitude": 45.0, "longitude": -93.0, "tags": []},
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"name": "Test", "latitude": "invalid", "longitude": -93.0, "tags": []},
            {"latitude": ["A valid number is required."]},
        ),
        (
            {"latitude": 45.0, "longitude": -93.0, "tags": []},
...
def test_read_anemometer_by_name(client, token):
    ...
    client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
    response = client.get("/api/anemometers/?search=Test")
    assert response.status_code == 200
...
def test_update_anemometer(client, token):
    ...
    anemometer = AnemometerFactory()
    update_data = {
        "name": "Updated Anemometer",
        "latitude": anemometer.latitude,
        "longitude": anemometer.longitude,
        "tags": ["updated"],
    }
    response = client.put(
        f"/api/anemometers/{anemometer.id}/", update_data, format="json"
    )
